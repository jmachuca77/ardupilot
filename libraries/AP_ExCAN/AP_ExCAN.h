...
#define EXCAN_MAX_NUM_ESCS 12

class CANTester;

class AP_ExCAN : public AP_CANDriver {
...
public:
    ...
    /* Do not allow copies */
    AP_ExCAN(const AP_ExCAN &other) = delete;
    AP_ExCAN &operator=(const AP_ExCAN&) = delete;

    // Return ExCAN from @driver_index or nullptr if it's not ready or doesn't exist
    static AP_ExCAN *get_excan(uint8_t driver_index);
...
private:
    ...
    static constexpr uint16_t EXCAN_OUTPUT_MIN = 6300;
    static constexpr uint16_t EXCAN_OUTPUT_MAX = 32000;
    static const uint16_t EXCAN_SEND_TIMEOUT_US;
    ...
    uint16_t _scaled_output[EXCAN_MAX_NUM_ESCS];
