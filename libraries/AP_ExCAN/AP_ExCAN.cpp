...
#define debug_can(level_debug, fmt, args...) do { AP::can().log_text(level_debug, "ExCAN",  fmt, #args); } while (0)


// stupid compiler is not able to optimise this under gnu++11
// move this back when moving to gnu++17
const uint16_t AP_ExCAN::EXCAN_SEND_TIMEOUT_US = 500;

AP_ExCAN::AP_ExCAN()
{
    debug_can(AP_CANManager::LOG_INFO, "ExCAN: constructed\n\r");
    (void)COMMAND_STOP;
    (void)MOTOR_DATA5;
}

AP_ExCAN *AP_ExCAN::get_excan(uint8_t driver_index)
{
    if (driver_index >= AP::can().get_num_drivers() ||
        AP::can().get_driver_type(driver_index) != AP_CANManager::Driver_Type_ExCAN) {
        return nullptr;
    }
    return static_cast<AP_ExCAN*>(AP::can().get_driver(driver_index));
}


bool AP_ExCAN::add_interface(AP_HAL::CANIface* can_iface) {
    if (_can_iface != nullptr) {
        debug_can(AP_CANManager::LOG_ERROR, "ExCAN: Multiple Interface not supported\n\r");
        return false;
    }
    ...
    if (_can_iface == nullptr) {
        debug_can(AP_CANManager::LOG_ERROR, "ExCAN: CAN driver not found\n\r");
        return false;
    }

    if (!_can_iface->is_initialized()) {
        debug_can(AP_CANManager::LOG_ERROR, "ExCAN: Driver not initialized\n\r");
        return false;
    }

    if (!_can_iface->set_event_handle(&_event_handle)) {
        debug_can(AP_CANManager::LOG_ERROR, "ExCAN: Cannot add event handle\n\r");
        return false;
    }
    return true;
}
...
// initialise ExCAN bus
void AP_ExCAN::init(uint8_t driver_index, bool enable_filters)
{
    _driver_index = driver_index;

    debug_can(AP_CANManager::LOG_DEBUG, "ExCAN: starting init\n\r");

    if (_initialized) {
        debug_can(AP_CANManager::LOG_ERROR, "ExCAN: already initialized\n\r");
        return;
    }

    if (_can_iface == nullptr) {
        debug_can(AP_CANManager::LOG_ERROR, "ExCAN: Interface not found\n\r");
        return;
    }

    // start calls to loop in separate thread
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AP_ExCAN::loop, void), _thread_name, 4096, AP_HAL::Scheduler::PRIORITY_MAIN, 1)) {
        debug_can(AP_CANManager::LOG_ERROR, "ExCAN: couldn't create thread\n\r");
        return;
    }

    _initialized = true;

    debug_can(AP_CANManager::LOG_DEBUG, "ExCAN: init done\n\r");
    gcs().send_text(MAV_SEVERITY_INFO,"ExCAN: init done");
    return;
}
...
{
    uint64_t timeout = 0;
    const uint32_t timeout_us = MIN(AP::scheduler().get_loop_period_us(), EXCAN_SEND_TIMEOUT_US);
    static uint32_t sendtext_timeref;

    while (true) {
        gcs().send_text_rate_limited(MAV_SEVERITY_INFO, 500, sendtext_timeref,"ExCAN Loop");

        if (!_initialized) {
            // if not initialised wait 2ms
            debug_can(AP_CANManager::LOG_DEBUG, "ExCAN: not initialized\n\r");
        ...
        if (send_stage == 5) {
            ...
            if (diff_ms >= EX_CAN_ESC_UPDATE_MS) {
                // set _telem_req_ms to time we ideally should have requested update
                if (diff_ms >= 2 * EX_CAN_ESC_UPDATE_MS) {
                    _telemetry_req_ms = now_ms;
                } else {
                    _telemetry_req_ms += EX_CAN_ESC_UPDATE_MS;
                }
        ...
        if (send_stage == 8) {
            ...
            while (read_frame(recv_frame, timeout)) {
                ...
                if ((recv_frame.id >= MOTOR_DATA1) && (recv_frame.id <= MOTOR_DATA1 + EXCAN_MAX_NUM_ESCS)) {
                ...
                if ((recv_frame.id >= MOTOR_DATA2) && (recv_frame.id <= MOTOR_DATA2 + EXCAN_MAX_NUM_ESCS)) {
                ...
                if ((recv_frame.id >= MOTOR_DATA3) && (recv_frame.id <= MOTOR_DATA3 + EXCAN_MAX_NUM_ESCS)) {
...
bool AP_ExCAN::write_frame(AP_HAL::CANFrame &out_frame, uint64_t timeout)
{
...
bool AP_ExCAN::read_frame(AP_HAL::CANFrame &recv_frame, uint64_t timeout)
{
...
void AP_ExCAN::update_esc_present_bitmask()
{
...
void AP_ExCAN::update()
{
    // take semaphore and update outputs
    {
        WITH_SEMAPHORE(_rc_out_sem);
        const bool armed = hal.util->get_soft_armed();
        for (uint8_t i = 0; i < MIN(EXCAN_MAX_NUM_ESCS, 16); i++) {
            ...
            if (!armed || (c == nullptr)) {
                ...
                if (pwm_out <= 1000) {
                    ...
                    _scaled_output[i] = EXCAN_OUTPUT_MAX;
                } else {
                    _scaled_output[i] = EXCAN_OUTPUT_MIN + (pwm_out - 1000) * 0.001f * (EXCAN_OUTPUT_MAX - EXCAN_OUTPUT_MIN);
                }
            }
        }
        update_count++;
    ...
    if (logger && logger->logging_enabled()) {
        ...
        for (uint8_t i = 0; i < MIN(EXCAN_MAX_NUM_ESCS, 8); i++) {
...
void AP_ExCAN::send_mavlink(uint8_t mav_chan)
{
    // compile time check this method handles the correct number of motors
    static_assert(EXCAN_MAX_NUM_ESCS == 12, "update AP_ExCAN::send_mavlink only handles 12 motors");
...
AP_ExCAN::motor_request_data_cmd_t AP_ExCAN::get_motor_request_data_cmd(uint8_t request_id) const
{
